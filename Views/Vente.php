...
<main>
                                ...
                                <?php foreach ($ventes as $vente) { ?>
                                    <tr class="item">
                                        ...
                                        <td class="d-flex flex-row ">
                                            ...
                                            <a href="#" class="btn btn-sm btn-warning updateVenteModel">
                                                <i class="fa fa-edit"></i>
                                                Edit</a>

                                        </td>
                                        <td>
                                            <a href="<?= BASE_URL ?>vente?DeleteVente=1&id_commande_client=<?= $vente['id_commande_client'] ?>"></a>
                                        </td>
                                    </tr>
                            </tbody>
                        <?php } ?>
    ...
    <!-- _____________________________________________End  Insert Model Vente_________________________________________________________ -->
    <!-- _______________________________________________________ Start uodate Modal Vente__________________________________________ -->
    ...
    <div class="modal fade" id="modalInsertVente" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalInsertVente">Update Vente</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form method="POST" class="needs-validation" novalidate>
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <input type="hidden" name="id_commande_client" value="<?= $vente['id_commande_client'] ?>">
                        </div>
                        <div class="form-group">
                            <label for="id_commande_client">reference Vente</label>
                            <input type="text" class="form-control" id="id_commande_client" name="id_commande_client" placeholder="reference Vente" required>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" name="UpdateVnete" class="btn btn-primary">Save data</button>
                    </div>
                </form>
            </div>
    ...
    <!-- _______________________________________________________ End uodate Modal Vente__________________________________________ -->

    <?php require_once('includes/footer.php'); ?>
    <script>
    //to show modal 
    document.querySelectorAll('.updateVenteModel').forEach(btn => {
        btn.addEventListener('click', e => {
            let item = e.target.closest('.item');
            const modal = document.querySelector('#updateVenteModel');
            console.log(item);
            console.log(modal);
            $("#updateVenteModel").modal('show')
        });
        //to update data 
        document.querySelectorAll('.updateVenteModel').forEach(btn => {
